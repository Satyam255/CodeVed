...
router.get("/", async (req, res) => {
  try {
    ...
    const loadTests = await LoadTest.find(filter)
      .sort({ timestamp: -1 })
      .skip(skip)
      .limit(limit);
...
router.get("/user/:clerkId", async (req, res) => {
  try {
    ...
    const loadTests = await LoadTest.find({ clerkId: req.params.clerkId })
      .sort({ timestamp: -1 })
      .skip(skip)
      .limit(limit);

    const total = await LoadTest.countDocuments({
...
router.get("/repo/:reponame", async (req, res) => {
  try {
    ...
    const loadTests = await LoadTest.find({ reponame: req.params.reponame })
      .sort({ timestamp: -1 })
      .skip(skip)
      .limit(limit);
...
router.put("/:id", async (req, res) => {
  try {
    const loadTest = await LoadTest.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
