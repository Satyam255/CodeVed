...
export default function CoursePage() {
  ...
  useEffect(() => {
    ...
    if (course.length > 0) {
      const totalSubtopics = course.reduce(
        (total, section) => total + section.subtopics.length,
        0
      );
      ...
      const completedSubtopics = course.reduce(
        (total, section) =>
          total +
          section.subtopics.filter((subtopic) => subtopic.completed).length,
        0
      );
                ...
                <div className="flex items-center justify-between py-6 px-8 border-b border-gray-800">
                  <div>
                    ...
                    <div className="flex items-center mt-2 space-x-2">
                      <Badge
                        variant="outline"
                        className="bg-indigo-950 text-indigo-300 border-indigo-800 px-2"
                      >
                        {title}
                      </Badge>
                      <Badge
                        variant="outline"
                        className="bg-gray-800 text-gray-300 border-gray-700 px-2"
                      >
                      ...
                      <div>
                        ...
                        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                          <p className="text-gray-300 leading-relaxed">
                            {videoSummary ||
                              "No summary available. Select a topic to view its summary."}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      {ques.length > 0 ? (
                        Object.values(ques).map((question: any) => (
                          <Card
                            key={question.id}
                            className="p-6 rounded-lg border-gray-800 bg-gray-800 shadow-lg"
                          >
